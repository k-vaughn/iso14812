...
from utils import get_qname, get_label, get_id, fmt_title, get_all_class_superclasses, is_refined_property, collect_list, get_class_expression_str, get_ontology_for_uri, insert_spaces, get_leaf_classes, get_property_info, collect_oneOf
...
def add_class_expression_node(graph, g: Graph, expr, ns: str, prefix_map: dict, global_all_classes: set, ns_to_ontology: dict, abstract_map: dict, created: set, is_superclass: bool = False, in_associated_cluster: bool = False, enum_members: list = None, enum_name: str = None) -> tuple:
    ...
    if isinstance(expr, URIRef):
        ...
        url = None if ':' in qname else f"../{label}/" if qname in global_all_classes else None
        label = get_label(g, expr)
        log.debug(f"Adding node for {qname}: label={label}, abstract={abstract_map.get(qname, False)}")
        graph.node(
            node_id,
