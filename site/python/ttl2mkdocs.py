...
from markdown_generator import generate_index, generate_markdown, generate_prop_list, update_mkdocs_nav
from ontology_processor_ttl import MASTER_FILE, process_vocabulary
from utils import fauxClass, get_id, get_label, get_qname, is_abstract
...
def main() -> None:
    ...
    try:
        ...
        g, ns, prefix_map, classes, local_classes, prop_map, global_patterns = result
        ...
        for cls in sorted(local_classes, key=lambda uri: get_label(g, uri).lower()):
            ...
            if not cls_name or cls_name == "ITSThing" or fauxClass(cls_name):
                continue
